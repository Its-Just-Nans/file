...
addEventListener("TrunkApplicationStarted", (event) => {
    ...
    const decodeFile = async (file) => {
        ...
        let json = window.wasmBindings.process_file(u8, filename);
        ...
        if (parsed?.file_type?.kind == "Image") {
            ...
            document.getElementById("dropHandler").appendChild(details3);
        } else if (parsed?.file_type?.kind == "Font") {
            let text = document.createElement("p");
            let fontName = "UploadedFont";
            text.id = "font";
            text.innerText = "The fox jumps over the lazy dog";
            text.style.fontSize = "2em";
            text.style.fontFamily = fontName;
            let style = document.createElement("style");
            style.innerHTML = `@font-face { font-family: "${fontName}"; src: url('${source}');}`;
            let details3 = document.createElement("details");
            let summary3 = document.createElement("summary");
            summary3.innerHTML = "Font";
            details3.appendChild(summary3);
            details3.appendChild(style);
            details3.appendChild(text);
            document.getElementById("dropHandler").appendChild(details3);
        }
    };
